...
get_q2_using_py <- function(k_vals, alpha, fold, iteration, seed_val, verbose = 0, paths_list_src_path) {
  ##
  if (verbose == 1) {
...
cv_model_select_pyNMF <- function(X,
  ...
  if (parallelDo) {
    ...
    if (useSLURM) {
      ...
      X <<- X
      cvfolds <<- cvfolds
      samarth_job <- slurm_apply(get_q2_using_py, grid_search_params,
                                 jobname = "grid_search_archR",
                                 nodes = nCores/16,
                                 cpus_per_node = 16,
...
get_best_alpha <- function(for_alpha, for_k, min_or_max = min) {
  ...
  #
  idx_best_alpha <- which(unlist(averagesA["q2_vals"]) > q2_threshold$mean)
  #
