collect_cluster_labels <- function(given_seqsClustLabels, choose_levels = 1){
    ...
    splitChar <- "-"
    elements_length <- unique(unlist(
        lapply(strsplit(given_seqsClustLabels, split = splitChar), length)
    ))
    # print(given_seqsClustLabels[1:10])
    # print(elements_length)
    # print(strsplit(given_seqsClustLabels[1:10], split = ""))
    # stop("SAMARTH")
    assertthat::are_equal(length(elements_length), 1)
    #
    # print(choose_levels)
    # print(elements_length)
    # print(choose_levels > elements_length)
    if (choose_levels > elements_length){
        stop(paste0("choose_levels(", choose_levels, 
                    ") greater than levels present in cluster_labels(",
                    elements_length, ").")
             )
    } else{
        selectedLabels <- 
            unlist(lapply(strsplit(given_seqsClustLabels, split = splitChar),
                          function(x){paste0(x[1:choose_levels], collapse=splitChar)}
                          )
                   )
    }
...
map_clusters_to_factors <- function(samplesMatrix, clustOrderIdx, iChunksColl,
                                    iChunkIdx, flags){
    #
    # clustOrderIdx IS clustering_sol_kmeans$reordering_idx
    # the reorderingIdx is a nested list, i.e. a list holding list of seqs_ids 
    # belonging to one cluster together. Hence, length(this variable) gives 
    # #clusters
    # 
    ...
    if (length(clustOrderIdx) == 1){
        ...
        print(clustOrderIdx)
        #stop("SAMARTH")
        rightClusterOrders <- vector("list", 
                                     length(clustOrderIdx))
        # for (cluster_idx in 1:length(clustOrderIdx)){
        #     relevant_factor <- which.max(
        #         rowMeans(as.matrix(samplesMatrix[,
        #                                          clustOrderIdx[[cluster_idx]]])
        #         )
        #     )
        #     if(flags$debugFlag){print(relevant_factor)}
        #     if(length(relevant_factor) > 1){
        #         if(flags$debugFlag){print("Factor-mapping NOT OK")}
        #     } else{
        #         if(flags$debugFlag){print("Factor-mapping OK")}
                rightClusterOrders[[1]] <- 
                    iChunksColl[[iChunkIdx]][clustOrderIdx[[1]]]
        #     }
        # }
        print("=== clustOrderIdx in map/aux function  IF==1 cond ===")
        print(clustOrderIdx)
...
collate_clusters <- function(hopachObj, globClustAssignments){
    ...
    ### globClustAssignments variable to combine the respective 
    ### sequences together.
    
    if(is.null(hopachObj)){
        # When HOPACH not performed, the global_cluster_assingments variable holds 
        # a list of lists, so we should flatten it with unlist()
        ...
        for (coll_cluster_idx in 1:length(collatedClustAssignments)){
            ...
            if (all(pick_this_itr <= length(globClustAssignments))){
                temp <- unlist(lapply(pick_this_itr,
                                      function(x){
...
update_cluster_labels <- function(oldSeqsClustLabels, 
    ...
    if(flags$verboseFlag){
        ...
        cat(paste0("#Clusters: ", 
                   length(collatedClustAssignments), "\n"))
    }
    ...
    for ( i in 1:length(collatedClustAssignments)){
        ...
        newSeqsClustLabels[ needUpdateIdx ] <- 
            sapply(newSeqsClustLabels[ needUpdateIdx ], 
                   function(x){
...
prepare_chunks <- function(total_avail, 
                           reqdChunkSize,
                           checkLength,
                           flags = list(debugFlag = TRUE,
                                        verboseFlag = TRUE,
                                        plotVerboseFlag = TRUE,
                                        timeFlag = TRUE)){
    ###
    ...
    if(length(total_avail) > reqdChunkSize){
        ...
        if(length(chunkStarts) > length(chunkEnds)){
            if(flags$debugFlag){print("Chunk starts/ends altered")}
...
handle_clustering_of_factors <- function(globFactorsMat,
                                         distMethod = "cosangle",
                                         flags){
    ### Currently relying on HOPACH algorithm
    ### - Compute cosine similarities (values between 0-1)
    ### - Using HOPACH algorithm for clustering with chosen distance 
    ### measure (we currently use 'cosangle' distance measure)
    ...
    if(doHopach[1] == 1 && ncol(globFactorsMat) > 2){
        ...
        pdf(paste0("samarth_test_factors_seqlogos.pdf"), 
                width = 11, 
                height = 1.5)
            for (i in 1:ncol(globFactorsMat)) {
                suppressMessages(
                viz_all_factors_as_seqlogo(as.matrix(globFactorsMat[,i]),
                                           position_labels = positions)
                )
            }
            dev.off()
    #}
    globFactorsHopach <- hopach::hopach(
        data = t(globFactorsMat), 
        dmat =  globFactorsDistMat,
        d = "cosangle", #default
        newmed = "uwnn", #default:medsil, uwnn
        clusters = "best",
        coll = "all", 
        initord = "clust", 
        #default is co; using clust saves reordering later, 
        #because the order given y hopach is already right (almost?)
        mss = "mean",
    ...
    if(flags$plotVerboseFlag){
        ### Order the medians, accordingly change order of the collated 
        ### cluster assignments
        medoidsIdx <- get_hopach_cluster_medoidsIdx(globFactorsHopach)
        ### Print medoid factors (or factors corresponding to clusters)
        pdf(paste0(prefix_str, 
                   "check_cluster_medoid_factors_seqlogos.pdf"), 
            width = 11, 
            height = 1.5)
        for (i in medoidsIdx) {
            viz_all_factors_as_seqlogo(as.matrix(globFactorsMat[,i]),
...
final_adjustment_of_ordering <- function(archRresult){
    # get hopach ordering of clusters
    # 
