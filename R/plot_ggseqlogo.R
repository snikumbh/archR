...
#' @import ggplot2
#' @import ggseqlogo
plot_ggseqlogo <- function(pwmMat, plotMethod = "custom", position_labels = NULL, savePDFfilename = NULL) {
    ...
    if (length(position_labels) < ncol(pwmMat)) {
        stop(paste0("Inadequate position labels supplied", ncol(pwmMat) - length(position_labels)))
    }
    # 
    if (length(position_labels) > ncol(pwmMat)) {
        stop(paste0("Overabundant position labels supplied", length(position_labels) - 
            ncol(pwmMat)))
    }
    # 
    p1 <- ggplot() + geom_logo(pwmMat, method = plotMethod, seq_type = "dna") + theme_logo() + 
        # theme_update(plot.margin=grid::unit(c(0,0,0,0), 'mm'), axis.ticks.length =
    # unit(0, 'lines'), axis.line = element_blank(), axis.title.y = element_blank(),
    # axis.text.y = element_blank() ) + theme_bw() +
    ggplot2::scale_x_continuous(breaks = 1:ncol(pwmMat), labels = position_labels, 
        expand = expand_scale(mult = c(0, 0))) + # ggplot2::coord_fixed(ratio = 8, clip = 'on') + ggplot2::labs(title = plot.title)
    # +
