...
#' #' @importFrom Biostrings DNAStringSet
#' #' @export
#' prepare_lowD_data_from_FASTA <- function(inputFastaFilename, rawSeq = FALSE,
#'                                     sinuc_or_dinuc_or_trinuc = "sinuc"){
#'
#'     if (file.exists(inputFastaFilename)) {
#'         givenFastaSeqs <-
#'             Biostrings::readDNAStringSet(filepath = inputFastaFilename,
...
#'     }
#'     #
#'     givenFastaSeqs <- Biostrings::DNAStringSet(toupper(givenFastaSeqs))
#'
#'     seqs_split_as_list <-
#'         base::strsplit(as.character(givenFastaSeqs), split = NULL)
#'
#'     dna_alphabet <- dna_alphabet <- c('A', 'C', 'G', 'T')
#'     dinuc <- get_dimers_from_alphabet(dna_alphabet)
#'     encoded_seqs <-
#'         lapply(seqs_split_as_list, function(givenSeq){
#'             given_seq_len <- length(givenSeq)
#'             givenSeq_dinuc <- unlist(lapply(seq_len(given_seq_len - 1), function(x) {
#'                 paste0(givenSeq[x], givenSeq[x + 1])
#'             }))
#'             encoded_mat <- matrix(rep(NA, given_seq_len-1), nrow = 1)
#'             for (i in seq_along(dinuc)){
#'                 encoded_mat[1, which(givenSeq_dinuc == dinuc[i])] = i-1
#'             }
#'             return(encoded_mat)
#'         })
#'     encoded_seqs <- do.call(rbind, encoded_seqs)
