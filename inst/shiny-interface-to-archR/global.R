...
#' Changes implemented:
#' 1. Directory creation in tempdir instead of current directory as in rslurm.
#' 2.
#'
#'
...
#' @param dir_path Path/location on disk where directory is to created.
...
archR_slurm_call <- function(f, params, jobname = NA, global_objects = NULL,
    ...
    if (is.null(names(params)) ||
        (!is.primitive(f) && !"..." %in% names(formals(f)) &&
         any(!names(params) %in% names(formals(f))))) {
    ...
    # Create temp folder
    tmpdirName <- paste0("_rslurm_", jobname)

    if(!is.null(dir_path)){
        tmpdirPath <- file.path(dir_path, tmpdirName)
    }else{
        tmpdirPath <- file.path(dir_path, tmpdirName)
    }
    ...
    saveRDS(params, file = file.path(tmpdirPath, "params.RDS"))
    saveRDS(f, file = file.path(tmpdirPath, "f.RDS"))
    if (!is.null(global_objects)) {
        save(list = global_objects,
             file = file.path(tmpdirPath, "add_objects.RData"),
             envir = environment(f))
    }
    ...
    script_r <- whisker::whisker.render(template_r,
                                        list(pkgs = pkgs,
                                             ...
                                             libPaths = libPaths))
    writeLines(script_r, file.path(tmpdirPath, "slurm_run.R"))
    ...
    writeLines(script_sh, file.path(tmpdirPath, "submit.sh"))
    ...
    if (submit) {
        jobid <- submit_slurm_job(tmpdirPath)
    } else {
        jobid <- NA
        cat(paste("Submission scripts output in directory", tmpdirPath,"\n"))
    }
...
#' This is an adapted version of the slurm_call function from rslurm package.
#' Changes implemented:
#' 1. Access custom directory created at custom location specified by the user
#'
#'
...
# Utility functions from rslurm package (not exported)
