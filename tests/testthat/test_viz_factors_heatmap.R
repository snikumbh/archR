context("Test factors visualized as heatmap")
library(ggplot2)
library(ggseqlogo)
...
test_that("Given object is matrix", {
  ...
  expect_error(viz_all_factors_as_heatmap(testFeaturesMat, position_labels = testPositionLabels)
               , "not of type matrix")
...
test_that("Handling empty matrix", {
  ...
  testPositionLabels <- seq(25)
  expect_error(viz_all_factors_as_heatmap(testFeaturesMat,
