...
test_that("Handles < 3 cross-validation folds", {
  ...
  testKFolds <- 1
  expect_error(cv_model_select_pyNMF(testDataX,
...
test_that("Handles erroneous colnames in input tibble (get_best_K)", {
  ...
  testParamRanges <- list(alphabase = 0, alphaPow = 0, k_vals = 0)
  testKFolds <- 5
  testModelSelectResult <- tibble::tibble(k_vals = seq(4), alpha = rnorm(4),
                                  fold = seq(4), q2vals = rnorm(4))
  expect_error(get_best_K(testModelSelectResult)
