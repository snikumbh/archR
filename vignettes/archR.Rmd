---
title: "Example usage with archR"
...
output:
  pdf_document:
    ...
    toc: yes
vignette: >
  %\VignetteIndexEntry{ExampleWitharchR}
...
# Introduction
archR is a non-negative matrix factorization (NMF)-based unsupervised learning approach for identifying different core promoter architectures. 
We demonstrated that archR's simple algorithm based on chunking and iterative processing successfully uncovers hidden clusters characterized by combinations 
of sequence motifs. 
...
A set of 1000 simulated DNA sequences, each 200 nucleotides long and with 
uniform probability for all nucleotides. These sequences have four clusters in 
them, each with 250 sequences. The profiles of the four clusters is as follows:
...
produce one-hot encoded DNA sequences.
Setting the argument `rawSeq`, the function returns the raw sequences.
...
represent_matrix_of_acgt_with_ggplot2(as.character(tssSeqs_raw), 
...
In the current version, archR naively returns a result object which is a list of
lists.
...
For each sequence, archR provides the cluster label for the sequence in each 
...
denotes the root (all data), the sequence was then assigned to cluster 3 in the 
...
subsequently, cluster 2.
For instance, below are the cluster labels for the first 100 sequences in the 
data.
...
### NMF basis vectors
archR also stores the NMF basis vectors corresponding to each cluster in every 
...
```{r visualize-basis-vectors-iter1, echo=TRUE, fig.height=2, fig.width=25, message=FALSE, warning=FALSE}
viz_all_factors_as_seqlogo(
...
viz_all_factors_as_heatmap(archRresult$clustBasisVectors[[2]]$basisVectors,
                           position_labels = positions)
```


```{r visualize-basis-vectors-combined, echo=TRUE, fig.height=4, fig.width=25, message=FALSE, warning=FALSE}
...
archR can detect de novo sequence features and simultaneously identify the 
...
archR. This potentially affects the computation time of archR. But being an 
embarrasingly parallelizable case, this issue is alleviated. There are already 
provisions in archR to choose parallelization for speeding up computations.
