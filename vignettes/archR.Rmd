...
This vignette demonstrates archR's usage with the help of a synthetic DNA 
sequences data set. 
...
If any points noted with regards to troubleshooting _archR_'s installation, please
file a [new issue](https://github.com/snikumbh/archR/issues/new).
...
The function `prepare_data_from_FASTA()` enables one-hot-encoding the DNA sequences in 
...
single nucleotide profiles is selected. See the `sinuc_or_dinuc` argument.


Upon setting the logical argument `rawSeq` to `TRUE`, the function returns the raw 
sequences as a `DNAStringSet` object, with `FALSE` it returns the column-wise 
...
`archR::viz_matrix_of_acgt_image` with the sorted order as shown.
```{r visualize-clusters-itr1, echo=TRUE, fig.height=6, fig.width=5, fig.cap="Result at iteration 1", eval=TRUE}
...
```{r visualize-clusters-itr2, echo=TRUE, fig.height=6, fig.width=5, fig.cap="Result at iteration 2", eval=TRUE}
