...
# Introduction
archR is a non-negative matrix factorization (NMF)-based unsupervised learning 
approach for identifying different core promoter architectures. We demonstrate 
in this vignette that archR's simple algorithm based on chunking and iterative 
processing successfully uncovers hidden clusters characterized by combinations 
of sequence motifs. 
...
If you already have a DNAStringSet object, you can use the 
...
The clustered output from _archR_ can again be visualized as a matrix. We simply 
sort the sequences based on the cluster labels assigned to them. Call 
`visualize_matrix_of_acgt` with the sorted order as shown.
```{r visualize-clusters, echo=TRUE, fig.height=6, fig.width=5, eval=FALSE}
sorted_order <- sort(collect_cluster_labels(archRresult$seqsClustLabels, 
                                            choose_levels = 3),
                    index.return = TRUE)
visualize_matrix_of_acgt(as.character(tssSeqs_raw[sorted_order$ix]), 
                         position_labels = positions,
                         saveFilename = NULL)
```
![archR-result-on-example-data.](./sample-clustering-from-archR-on-example-data.png)

The above output was generated by performing 100 bootstrapped iterations, 
parallely by specifying to _archR_ 
