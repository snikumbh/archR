...
resource_files:
  - reference/figures/archR_algorithm_static_illustrator_cropped.png
  - reference/figures/archR_algorithm_1080p_cropped.gif
---
...
# _archR_'s algorithm
_archR_ implements a chunking-based iterative procedure. Below is a schematic of archR's algorithm.

<img src="reference/figures/archR_algorithm_static_illustrator_cropped.png" width="550" align="center">
...
Once the configuration is setup, call the `archR::archR` function with user-specified 
iterations.
...
2. the collection of NMF basis vectors per iteration [`clustBasisVectors`]: each 
...
clusters from the last iteration of archR. This element stores the clustering of 
NMF basis vectors [`basisVectorsClust`] and the sequence clusters [`clusters`]; 
4. the raw sequences provided [`rawSeqs`]; 
5. the configuration setting [`config`]; and 
6. the call itself [`call`].
...
```{r visualize-clusters-itr1, echo=TRUE, fig.height=6, fig.width=5, fig.cap="Figure: Clusters at iteration 1", eval=TRUE}
...
```{r visualize-clusters-itr2, echo=TRUE, fig.height=6, fig.width=5, fig.cap="Figure: Clusters at iteration 2", eval=TRUE}
