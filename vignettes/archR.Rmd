...
processing. 
While matrix factorization approaches (and their applications) are known to 
scale poorly for large amounts of data, this approach enables scalable 
processing of large number of sequences.
To our knowledge, this is a novel application of NMF on sequence data.
Sequence motifs, their positional specificities and complex 
inter-relationships, all are simultaneously inferred from the data. 


This vignette demonstrates archR's usage. 
Please refer to the paper (cite paper/preprint) for a description and 
discussion of _archR_'s algorithm.
...
A set of 1000 simulated DNA sequences was generated, each 200 nucleotides long 
and with uniform probability for all nucleotides. 
These sequences have four clusters in them, each with 250 sequences. 
...
positions <- seq(1, Biostrings::width(inputSeqsRaw))
...
# Visualize the sequences in a image matrix where the bases are assigned fixed
# colors

archR::viz_matrix_of_acgt_image(as.character(inputSeqsRaw), 
...
### You can use parallelization by setting 'parallelize = TRUE' and 
### 'nCoresUse = n', where 'n' is the number of cores to be used.
...
In the current version, _archR_ naively returns a result object which is a list 
of lists.
The elements include: (a) the sequence cluster labels per iteration; (b) the 
collection of NMF basis vectors per iteration; (c) the raw sequences provided; 
(d) the configuration setting used; and (e) the call itself.



### NMF basis vectors
_archR_ also stores the NMF basis vectors corresponding to each cluster in 
...
```{r visualize-clusters, echo=TRUE, fig.height=6, fig.width=5, eval=TRUE}
...
archR::viz_matrix_of_acgt_image(as.character(inputSeqsRaw[sorted_order$ix]), 
                         position_labels = positions,
                         savefilename = NULL)
```
