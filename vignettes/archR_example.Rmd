...
vignette: >
  ...
  %\VignetteIndexEntry{Example usage archR}
output:
  html_document: 
    fig_caption: yes
    keep_md: yes
...
```{r visualize-basis-vectors, fig.height=2, fig.width=25, echo=TRUE, message=FALSE, warning=FALSE}
viz_all_factors_as_seqlogo(as.matrix(archRresult$clustBasisVectors[[2]]$BasisVectors), 
...
### Visualizing sequence clusterings at any iteration
We can visualize the sequence clusterings at any iteration of archR as follows.

```{r visualize-seqs-clusters, echo=TRUE, message=FALSE, warning=FALSE}

iupac_colorings <- c("A" = "darkgreen", "C" = "blue", "G" = "orange",
                     "T" = "red")
# for(i in 1:length(archRresult$clustFactors)){
#     chosenLevelLabels <- archR::collect_cluster_labels(archRresult$seqsClustLabels,
#                                                 choose_levels = i + 1)
#     chosenLevelLabels_sorted <- sort(chosenLevelLabels, index.return = TRUE)
#     # fname <- paste0("samarth_all_seqs_Level", i,"_Clustering")
#     represent_matrix_of_acgt_with_ggplot2(tssSeqs[chosenLevelLabels_sorted$ix],
#                                           position_labels = positions,
#                                           sinuc_or_dinuc = "sinuc",
#                                           choose_colors = iupac_colorings,
#                                           annClusters = chosenLevelLabels_sorted$x
#                                           # saveFilename = fname,
#                                           # fileType = "png"
#     )
#     
