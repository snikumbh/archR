...
```{r plot-seqs, echo=TRUE}
# Represent sequences as heatmap
represent_matrix_of_acgt(tssSeqs, position_labels = positions)

```

# Run model selection for NMF
...
```{r pyNMF-on-data, echo=TRUE, warning=FALSE, message=FALSE}
param_ranges <- NULL
...
start <- Sys.time()
chooseK_logfilename <- file.path(
  paste0('pyNMF-synthetic-data2-worker-output-chooseK-', paste(Sys.Date(), 
                                                               format(Sys.time(),
                                                                      "%X"), 
                                                               sep='-'), 
         '.txt'))
model_selectK <- invisible(cv_model_select_pyNMF(X=tssSeqs, 
...
q2_means_by_k_vals <- get_q2_aggregates_chosen_var(model_selectK, 
...
seed_val <- 10208090
start <- Sys.time()
chooseAlpha_logfilename <- file.path(
  paste0('pyNMF-synthetic-data2-worker-output-chooseAlpha-', paste(Sys.Date(), 
                                                                   format(
                                                                     Sys.time(),
                                                                     "%X"), 
                                                                   sep = '-'), 
...
model_selectAlpha <- invisible(cv_model_select_pyNMF(X = tssSeqs,
                                                     param_ranges = 
                                                       param_ranges,
                                                     kFolds = kFolds,
                                                     parallel = FALSE,
                                                     ...
                                                     seed_val = seed_val,
                                                     logfile = 
                                                       chooseAlpha_logfilename,
                                                     set_verbose = 0
)
)
print(Sys.time() - start)

best_alpha <- get_best_alpha(for_alpha = model_selectAlpha, 
                             for_k = model_selectK, 
                             min_or_max = min)
q2_means_by_alpha <- get_q2_aggregates_chosen_var(model_selectAlpha, 
...
# Perform NMF on complete data

```{r final-nmf-on-data, echo=TRUE}
cat("Performing NMF on the complete data...")
seed_val <- 10208090
start <- Sys.time()
result <- perform_nmf_func(tssSeqs, 
...
```{r fetch-factorized-matrices, echo=TRUE}
featuresMatrix <- result[[1]]
samplesMatrix <- result[[2]]

```

## Possibility of sparsification

```{r sparsify-feature-matrix, fig.height=2, fig.width=25, echo=TRUE, message=FALSE, warning=FALSE}
...
# Heatmap and sequence logos -- combined plots 
## The non-sparsified features matrix
```{r combined-plot-patterns-as-PWMs-non-sparse, fig.height=5, fig.width=25, echo=TRUE, warning=FALSE, message=FALSE}
# Plot here and also save as PDF -- both done in the function
# filename_heatmaps <- "architectures-as-patterns-pyNMF-sparsified-heatmaps.pdf"
# viz_all_factors_as_heatmap(featuresMatrix)
# #
# filename <- "architectures-as-patterns-pyNMF-non-sparsified.pdf"
# viz_all_factors_as_seqlogo(featuresMatrix)
#
viz_all_factors_in_combined_heatmaps_seqlogos(featuresMatrix = featuresMatrix, 
                                              add_pseudo_counts = F, 
...
```{r Cluster-sequences, echo=TRUE, warning=FALSE, message=FALSE}

final_nClust <- choose_clusters(givenMatrix = samplesMatrix, 
                                clustMethod = "kmeans", 
                                nClusterValsTest = seq(3,20))
cat("Number of clusters: ", final_nClust, "\n")

clustering_sol <- get_clusters(givenMat = samplesMatrix, 
                               clustMethod = "kmeans", 
                               nCluster = final_nClust)

```

# Represent clustered sequences as an image

```{r plot-seqs-post-nmf, fig.align = "center", echo=TRUE}

represent_matrix_of_acgt(givenMat = tssSeqs[ , unlist(clustering_sol$reordering_idx)], 
                         position_labels = positions, 
                         plot.title = "NMF+kmeans")

```

# Report architectures characteristic of clusters

```{r report-arch, fig.height=7, fig.width=25, echo=TRUE, warning=FALSE, message=FALSE}

plot_arch_for_clusters(givenSamplesMatrix =  samplesMatrix, 
                       givenFeaturesMatrix = featuresMatrix, 
                       nCluster = final_nClust, 
                       clustering_sol = clustering_sol, 
                       seqs = tssSeqs, 
                       position_labels = positions)
