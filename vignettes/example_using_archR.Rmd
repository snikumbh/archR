...
```{r load-example-data, echo=FALSE}
...
#
viz_all_factors_in_combined_heatmaps_seqlogos(featuresMatrix, add_pseudo_counts = F, position_labels = positions)
```
...
final_nClust <- choose_clusters(samplesMatrix, clustMethod = "kmeans", nClusterValsTest = seq(3,20))
cat("Number of clusters: ", final_nClust, "\n")

clustering_sol <- get_clusters(samplesMatrix, clustMethod = "kmeans", final_nClust)
...
represent_matrix_of_acgt(tssSeqs[ , unlist(clustering_sol$reordering_idx)], positions, plot.title = "NMF+kmeans")
...
plot_arch_for_clusters(samplesMatrix, featuresMatrix, final_nClust, clustering_sol, tssSeqs, positions)
