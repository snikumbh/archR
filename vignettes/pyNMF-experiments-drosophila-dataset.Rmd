---
title: "pyNMF-experiments-Drosophila-carcass-90bp-data"
author: "snikumbh"
date: "`r Sys.time()`"
output:
  html_document: 
...
inputFastaFilename <- file.path(paths_list$drosophila_data_path, 'flanked-drosophila-S1-CAGE-Dmel-FM-carcass-45U-45D.fa')
tss.seqs_o <- prepare_data_from_FASTA(inputFastaFilename)
nSeqs <- ncol(tss.seqs_o)
positions <- seq(-45,45) #setdiff(seq(-45,45), 0)
sinuc <- c('A', 'C', 'G', 'T')
tss.seqs <- tss.seqs_o
...
final_nClust <- choose_clusters(t(samplesMatrix), clustMethod = "hclust", nCluster_vals_test = seq(3,50))
cat("Number of clusters: ", final_nClust, "\n")

clustering_sol_hclust <- get_clusters(t(samplesMatrix), clustMethod = "hclust", final_nClust)
...
represent_matrix_of_acgt(tss.seqs, positions = positions, plot.title = "Before")
#represent_matrix_of_acgt(tss.seqs[ , unlist(clustering_sol$reordering_idx)], positions, plot.title = "NMF+kmeans")
represent_matrix_of_acgt(tss.seqs[ , unlist(clustering_sol_hclust$reordering_idx)], positions, plot.title = "NMF+hclust")
